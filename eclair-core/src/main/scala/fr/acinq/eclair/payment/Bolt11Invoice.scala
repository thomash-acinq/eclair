...
import fr.acinq.eclair.{CltvExpiryDelta, FeatureSupport, Features, MilliSatoshi, MilliSatoshiLong, TimestampSecond, randomBytes32}
 ...
 */
case class Bolt11Invoice(prefix: String, amount: Option[MilliSatoshi], timestamp: TimestampSecond, nodeId: PublicKey, tags: List[Bolt11Invoice.TaggedField], signature: ByteVector) extends PaymentRequest {
  ...
  lazy val relativeExpiry: Long = tags.collectFirst {
  ...
  def write: String = {
    ...
    val hramount = Amount.encode(amount)
...
object Bolt11Invoice {
  ...
  object Codecs {
    ...
    val taggedFieldCodec: Codec[TaggedField] = discriminated[TaggedField].by(ubyte(5))
      ...
      .typecase(5, dataCodec(bits).xmap[Features](Features(_), _.toByteVector.bits).as[InvoiceFeatures])
